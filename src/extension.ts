...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Nouvelle commande : sélectionner le dossier de backup
  const disposableSelectBackupFolder = vscode.commands.registerCommand('extension.selectBackupFolder', async () => {
    await selectBackupFolder();
  });

  // On enregistre toutes les commandes créées
  context.subscriptions.push(
    disposableChat,
    disposableSaveMe,
    disposableShowLog,
    disposableShowBackups,
    disposableSelectBackupFolder
  );
}
...
/**
 * Appelle l'API Mistral pour obtenir la réponse (ex. https://api.mistral.ai/v1/chat/completions).
 */
  ...
  if (mainPanel) {
    ...
    mainPanel.webview.onDidReceiveMessage(async (message) => {
      switch (message.command) {
        ...
        case 'selectBackupFolder': {
          // On invoque la commande extension.selectBackupFolder
          vscode.commands.executeCommand('extension.selectBackupFolder');
          break;
        }
        default:
          break;
 ...
 * Ajout d'un bouton "Choisir Dossier Backup".
 */
...
<body>
  ...
  <div>
    ...
    <button onclick="chooseBackupFolder()">Choisir Dossier Backup</button>
  </div>
  ...
  <script>
    ...
    function chooseBackupFolder() {
      vscode.postMessage({ command: 'selectBackupFolder' });
    }
...
/**
 * Sélection du dossier de backup : ouvre une boîte de dialogue, met à jour la config.
 */
async function selectBackupFolder() {
  const folderUri = await vscode.window.showOpenDialog({
    ...
    openLabel: 'Sélectionner le dossier de backup'
  });

  if (folderUri && folderUri[0]) {
    const selectedFolder = folderUri[0].fsPath;
    await vscode.workspace.getConfiguration('mistralExtension').update('backupFolder', selectedFolder, vscode.ConfigurationTarget.Workspace);
    addLog(`Dossier de backup mis à jour: ${selectedFolder}`);
    vscode.window.showInformationMessage(`Dossier de backup mis à jour: ${selectedFolder}`);
  }
}
